...
type DeliveryChallan struct {
	...
	DCType           string     `json:"dc_type"` // "transit" or "official"
...
func (dc *DeliveryChallan) Validate() map[string]string {
	...
	if dc.ChallanDate == nil || strings.TrimSpace(*dc.ChallanDate) == "" {
		errors["challan_date"] = "DC date is required"
	}
	if dc.DCType != "transit" && dc.DCType != "official" {
		errors["dc_type"] = "DC type must be 'transit' or 'official'"
	}
