...
func EditWizardStep2(c echo.Context) error {
	...
	// Load preselected ship-to IDs from the group's existing official DCs.
...
// EditWizardStep3 processes Step 2 of the edit wizard and renders Step 3 (serial entry)
// with the group's current transit-DC serial numbers pre-filled.
func EditWizardStep3(c echo.Context) error {
	...
	// Load ship-to address details (filter to selected ones)
	shipToConfig, _ := database.GetOrCreateAddressConfig(projectID, "ship_to")
	...
	// Build ship-to IDs string slice for hidden fields
	...
	return components.RenderOK(c, layouts.MainWithContent("Edit Shipment — Step 3", sidebar, topbar, "", "", pageContent))
}

// EditWizardStep4 processes Step 3 of the edit wizard and renders Step 4 (review).
func EditWizardStep4(c echo.Context) error {
	...
	sidebar := partials.Sidebar(user, project, allProjects, c.Request().URL.Path)
	topbar := partials.Topbar(user, project, allProjects, "", "")
	return components.RenderOK(c, layouts.MainWithContent("Edit Shipment — Step 4: Review", sidebar, topbar, "", "", pageContent))
}
...
// serial number slices for the transit DC.
func buildTransitLineItems(products []*models.TemplateProductRow, serialData []pageshipments.WizardSerialData, numSets int) ([]models.DCLineItem, [][]string) {
	items := make([]models.DCLineItem, 0, len(products))
	serialsByLine := make([][]string, 0, len(products))
	for _, p := range products {
		qty := p.DefaultQuantity * numSets
...
func buildOfficialLineItems(products []*models.TemplateProductRow) []models.DCLineItem {
	items := make([]models.DCLineItem, 0, len(products))
	for _, p := range products {
		items = append(items, models.DCLineItem{
			ProductID: p.ID,
...
func SaveShipmentEdit(c echo.Context) error {
	...
	transitLineItems, transitSerialsByLine := buildTransitLineItems(products, serialData, numSets)
	officialLineItems := buildOfficialLineItems(products)
	...
	if err := database.UpdateShipmentGroup(gid, &templateIDCopy, numSets, taxType, reverseCharge); err != nil {
		return handleEditError(c, project.ID, gid, err)
	}
	...
	for _, addressID := range toUpdate {
		...
		if err := database.ReplaceLineItemsAndSerials(dc.ID, project.ID, officialLineItems, nil); err != nil {
			return handleEditError(c, project.ID, gid, err)
		}
	}
